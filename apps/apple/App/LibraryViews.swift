...
extension BookmarkView {
    var displayTitle: String { preview?.title.flatMap { $0.isEmpty ? nil : $0 } ?? value.subject }
    var readingMinutes: Int { max(2, min(12, Int((Double((displayTitle + " " + (preview?.description ?? "")).utf16.count) / 140).rounded()) + 2)) }
    var contentFilter: ContentFilter {
        ...
        let url = URL(string: subject)
        let last = url?.lastPathComponent ?? ""
        if !displayTitle.lowercased().hasPrefix("saved from "), preview?.title != nil,
